...
class MockProgressWriter:
    def __init__(self, trainer, expected_cv=None, training_summary_counter=0):
...
def test_session_cross_validation_at_end(tmpdir, device_id):
    ...
    writer = MockProgressWriter(t['trainer'], expected_cv=[[92, 25]])
    session = training_session(mbs, t['trainer'], minibatch_size_schedule(4),
                               model_inputs_to_mb_source_mapping=input_map,
...
def test_session_cross_validation_3_times(tmpdir, device_id):
    ...
    writer = MockProgressWriter(t['trainer'], expected_cv=[[92, 25], [92, 25], [92, 25]])
    session = training_session(mbs, t['trainer'], minibatch_size_schedule(4),
                               model_inputs_to_mb_source_mapping=input_map,
...
def test_session_progress_print(tmpdir, device_id):
    ...
    writer = MockProgressWriter(t['trainer'])
    session = training_session(
        training_minibatch_source=mbs,
        trainer=t['trainer'],
        mb_size_schedule=minibatch_size_schedule(4),
...
def test_session_restart_from_checkpoint(tmpdir, device_id):
    ...
    writer2 = MockProgressWriter(t['trainer'], training_summary_counter=1)
    session = training_session(
        training_minibatch_source=mbs,
        trainer=t['trainer'],
        mb_size_schedule=minibatch_size_schedule(4),
    ...
    session.train(device)
    candidates = [f for f in listdir(test_dir) if
                  isfile(join(test_dir, f)) and f.startswith("saved_restart_from_checkpoint0")]
